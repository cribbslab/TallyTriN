...
def extract_umi_from_polyA_region(seq, n_trimers):
    
    UMI_TRIMERS = n_trimers
    UMI_LENGTH = UMI_TRIMERS * 3
...
def process_umi_sequence(seq, n_trimers):
    seq = seq.upper()
    umi, errors, best_trim_corr, best_base_counts, best_event_counts = extract_umi_from_polyA_region(seq, n_trimers)
...
def main():
    parser = argparse.ArgumentParser(description="Extract and correct UMI from sequence after polyA region.")
    parser.add_argument("--seq", required=True, help="Sequence following the polyA tail.")
    parser.add_argument("--n_trimers", type=int, default=12, help="Number of trimers to examine (default: 12).")
    args = parser.parse_args()

    process_umi_sequence(args.seq, args.n_trimers)
