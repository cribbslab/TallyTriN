...
def extract_umi_from_polyA_region(seq):
    ...
    #if len(remaining) < 30 or len(remaining) > 45:
...
# ------------------ Argument Parsing ------------------
parser = argparse.ArgumentParser(description="Debug adapter matches and UMI extraction using Parasail")
parser.add_argument("--infile", required=True, help="Input gzipped FASTQ file (.fastq.gz)")
parser.add_argument("--outfile", default="umi_reads.fastq", help="Output FASTQ for reads with UMI")
parser.add_argument("--umifile", default="umi_list.txt", help="Output file listing one UMI per line")
parser.add_argument("--tsvfile", default="umi_debug_output.tsv", help="TSV file with UMI extraction details")
parser.add_argument("--logfile", default="adapter_match_parasail_debug.log", help="Log file for match info")
parser.add_argument("--no_adapter_logfile", default="no_adapter_scores.log", help="Log for reads without adapter")
parser.add_argument("--no_adapter_outfile", default="no_adapter.fastq", help="Reads without adapter")
parser.add_argument("--no_umi_outfile", default="no_umi.fastq", help="Reads with adapter but no UMI extracted")
parser.add_argument("--max_reads", type=int, default=100000, help="Number of reads to test")
...
no_adapter_logger = logging.getLogger("no_adapter_logger")
no_adapter_handler = logging.FileHandler(args.no_adapter_logfile)
no_adapter_handler.setLevel(logging.INFO)
no_adapter_logger.addHandler(no_adapter_handler)
...
with pysam.FastxFile(args.infile) as fh, \
     open(args.outfile, "w") as out_fastq, \
     open(args.umifile, "w") as umi_out, \
     open(args.tsvfile, "w") as debug_tsv, \
     open(args.no_adapter_outfile, "w") as no_adapter_out, \
     open("umi_seq_length_outliers.txt", "w") as outlier_log, \
     open(args.no_umi_outfile, "w") as no_umi_out:

    debug_tsv.write("read_name\tumi\ttotal_errors\tumi_seq_used\n")
    ...
    for n, record in enumerate(fh, 1):
        ...
        if result.score >= min_score_threshold:
            ...
            if umi_start >= 0:
                ...
                #umi, total_errors, used_seq = extract_umi_from_polyA_region(umi_window)
                #umi_candidate_lengths[len(used_seq)] += 1
                if used_seq:
                ...
                umi_pass = False
                if umi:
                    umi_validation = parasail.sw_trace_striped_16(umi, used_seq, 1, 1, matrix)
                ...
                if umi_pass == True:
                    umi_extracted += 1
                    new_name = f"{record.name}_UMI_{umi}"
...
Adapter matched            : {adapter_matched}
UMI successfully extracted : {umi_extracted}
Reads without adapter      : {adapter_missing}
Adapter matched but no UMI : {umi_failed}
"""
print(summary)

print("\nUMI Candidate Sequence Length Histogram")
print("=======================================")
print(f"Short (<30bp): {short_count}")
for length, count in sorted(umi_candidate_lengths.items()):
    print(f"Length {length}: {count} reads")
print(f"Long (>45bp): {long_count}")


print("\nError-counts histogram (total errors):")
print("-------------------------------------------------")
for total_errors, count in sorted(error_count.items()):
    print(f"Total errors {total_errors}: {count} reads")

#print("\nError-type combination histogram (subs, ins, dels):")
#print("-------------------------------------------------")
#for (subs, ins, dels), count in error_patterns.most_common():
#    print(f"{subs} subs, {ins} ins, {dels} dels : {count} reads")

# Log the error type histogram to the log file
print("\nError Type Histogram (Insertions, Deletions, Substitutions):\n")
print("---------------------------------------------------------------\n")
for error_type, count in sorted(error_type_counter.items()):
    print(f"{error_type.capitalize()}: {count} reads\n")
...
for score, count in sorted(umi_score_counter.items()):
    print(f"UMI score {score}: {count} reads")
print(f"\nUmi-alignment score < {VALIDATION_THRESHOLD}: ({low_score})")
with open("summary_counts.txt", "w") as summary_file:
    summary_file.write(summary)

print(f"\nadapters not starting with G = {G_count}")
...
max_memory_kb = usage.ru_maxrss  # In kilobytes on Linux. On macOS, it's in bytes.

# If you're on macOS, convert bytes to kilobytes:
import platform
if platform.system() == "Darwin":
    max_memory_kb = max_memory_kb / 1024

print(f"Max memory usage = {max_memory_kb:.2f} KB")
