...
class protDTSplit(Config.config):
    ...
    def __init__(self, fpns):
        ...
        for method, fpn in fpns.items():
            ...
            df_met_T.columns = ['{:.1e}'.format(x) for x in self.seq_errs[13:]]
            # df_met_T.columns = [int(x*100000) for x in self.seq_errs[13:]]
            ...
            df_met['metric'] = ['{:.1e}'.format(x) for x in self.seq_errs[13:]]
